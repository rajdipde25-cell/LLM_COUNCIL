...
[![CI](https://github.com/tashfeenahmed/llm_council/actions/workflows/ci.yml/badge.svg)](https://github.com/tashfeenahmed/llm_council/actions/workflows/ci.yml)
...
The problem is that stacking them by hand is painful: fourteen different SDKs, fourteen different rate limits, fourteen places a request can fail. LLM_COUNCIL collapses that into one OpenAI-compatible endpoint. Point any OpenAI client library at your local server, and it routes transparently across whichever providers you've added keys for.
...
- **Unified API key** — Clients authenticate to your proxy with a single `llm_council-…` bearer token. You never expose upstream provider keys to your apps.
...
```bash
git clone https://github.com/tashfeenahmed/llm_council.git
cd llm_council
npm install
...
client = OpenAI(
    ...
    api_key="llm_council-your-unified-key",
)
...
curl http://localhost:3001/v1/chat/completions \
  -H "Authorization: Bearer llm_council-your-unified-key" \
...
Thanks to everyone who's helped improve LLM_COUNCIL:
...
**This project is for personal experimentation and learning, not production.** Free tiers exist so developers can prototype against them; they aren't a stable, supported inference substrate and shouldn't be treated as one. If you build something real on top of LLM_COUNCIL, swap in a paid API before you ship. Your relationship with each upstream provider is governed by the terms you accepted when you created your account — those terms still apply when the traffic is proxied through this project, and you're responsible for complying with them.
